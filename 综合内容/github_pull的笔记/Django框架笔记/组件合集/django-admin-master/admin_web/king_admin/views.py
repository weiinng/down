...
def index(request):
    ...
    request.session["url"] = {'/king_admin/admin_a/ip_host/': 'ip表', '/king_admin/admin_a/userinfo/': '用户表',
                              '/index/': '仪表盘'}
...
def table_obj(request, app_name, table_name):
    ...
    data_list = admin_class.module.objects.filter(**filter_conditions).order_by(
        "%s" % admin_class.ordering if admin_class.ordering else "-id")  # 过滤后的结果
    ...
    paginator = Paginator(list(set(data_list)), admin_class.page_number)  # 每页显示2个 并且把数据传入进来
    ...
    if current_page == 1:
        ...
        prev = '<li><a href="%spage=%s&o=%s&q=%s" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>' % (
        filter_str, current_page - 1, o, q_str)
    ...
    for i in range(begin + 1, end_num + 1):
        ...
        temp = '<li class="%s"><a href="%spage=%s&o=%s&q=%s"> %s <span class ="sr-only"></span ></a></li>' % (
        active, filter_str, i, o, q_str, i)
        list_page.append(temp)

    if current_page == all_page:
        down = '<li class ="disabled"><a href="#" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>'
    else:
        down = '<li><a href="%spage=%s&o=%s&q=%s" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>' % (
        filter_str, current_page + 1, o, q_str)
    ...
    return render(request, 'king/king_table.html',
                  {'admin_class': admin_class, 'contacts': contacts, 'str_page': mark_safe(temp),
...
def table_obj_change(request, app_name, table_name, first_field):
    ...
    return render(request, 'king/king_table_change.html',
                  {'form_obj': form_obj, 'app_name': app_name, 'table_name': table_name,
                   "admin_class": admin_class, "get_uri": referer})
...
def table_obj_add(request, app_name, table_name):
    ...
    if request.method == "POST":
        ...
        form_obj = form_class()
    return render(request, 'king/king_table_add.html',
                  {'form_obj': form_obj, 'app_name': app_name, 'table_name': table_name,
...
def table_obj_delete(request, app_name, table_name, obj_id):
    ...
    return render(request, 'king/king_table_delete.html',
                  {'admin_class': admin_class, 'objs': [obj, ], 'app_name': app_name,
